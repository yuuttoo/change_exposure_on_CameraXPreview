...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setupPermissions()
        //viewFinder.setOnClickListener(HorizontalScrollListener())
    }
    ...
    private fun bindCameraUseCases() {
        ...
        try {
            ...
            viewFinder.setOnTouchListener(HorizontalScrollListener())
    ...
    // View.OnTouchListener to handle horizontal scroll for exposure adjustment
    private inner class HorizontalScrollListener : View.OnTouchListener {
        ...
        override fun onTouch(view: View, event: MotionEvent): Boolean {
            when (event.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    camera?.let { adjustExposure(it, deltaX) }
                    return true
                }
            }
            return false
        ...
        @SuppressLint("UnsafeOptInUsageError")
        private fun adjustExposure(camera: Camera, deltaX: Float) {
            camera.cameraInfo.exposureState.let {
                val sensitivity = 0.1
                val exposureChange = deltaX * sensitivity

                // Get the current exposure
                val currentExposure = camera.cameraInfo.exposureState.exposureCompensationIndex.toFloat()

                // Calculate the new exposure value
                val newExposure = (currentExposure + exposureChange).coerceIn(
                    camera.cameraInfo.exposureState.exposureCompensationRange.lower.toFloat().toDouble(),
                    camera.cameraInfo.exposureState.exposureCompensationRange.upper.toFloat().toDouble()
                )

                // Set the new exposure using CameraControl
                camera.cameraControl.setExposureCompensationIndex(newExposure.roundToInt())
                Log.i("newExposure:", "${newExposure.roundToInt()}" )
            }
            }
        }
